...
class BPNN():
    ...
    def __init__(self, train_in, train_out, learn_rate=0.03, son_samples=50, iter_times=1000, hidden_layer=[20,20],middle_name='Sigmoid', last_name='Sigmoid', cost_func='Lsm', norr=0.00002, break_error=0.0001):
    ...
    def train_adam(self,key, mom=0.9, prop=0.9):
        # 采用随机小批量梯度下降
        # 首先结合输入和输出数据
        alldata = np.hstack((self.train_in, self.train_out))
        np.random.shuffle(alldata)  # 打乱顺序
        ...
        while iter < self.iter_times:
            ...
            # 打印误差值
            errortrain = eval(self.cost_func)(trout, z[len(self.all_layer) - 1])
            print(key+': '+u'第%s代总体误差：%.9f' % (iter, errortrain))
...
if __name__ == "__main__":
    errordic = {}
    for key in R_data:
        train_x_data = R_data.get(key)[0]  # 训练输入
        # np.array(InputHandler(R_data.get(key)[0],LL))
        train_y_data = R_data.get(key)[1]  # 训练输出
        predict_x_data = R_data.get(key)[2]  # 测试输入
        # np.array(InputHandler(R_data.get(key)[2],LL))
        predict_y_data = R_data.get(key)[3]  # 测试输出
        # 开始训练数据
        bpnn = BPNN(train_x_data, train_y_data)
        bpnn_train = bpnn.train_adam(key)

        joblib.dump(bpnn, 'model/'+key+'.model')

        minumber = R_data.get(key)[4][1]
        maxumber = R_data.get(key)[4][0]
