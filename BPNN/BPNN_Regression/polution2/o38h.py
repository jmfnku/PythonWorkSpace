...
#  构建训练函数
def Ten_train(xdata, ydata,prexdata,key,hiddenlayers=1, hiddennodes=8, \
              learn_rate=0.001, itertimes=20000, batch_size=2, activate_func='sigmoid', break_error=0.005):
    # 开始搭建神经网络
...
def costfig(errlist, le='成本函数曲线图'):
    ...
    plt.ylabel('成本函数值')
    plt.show()
def batchSizefigure(aucdict,le='batchSize分析图'):
    key = []
    val = []
    for k in aucdict:
        key.append(k)
        val.append(aucdict.get(k))
    plt.plot(key,val)
    plt.title(le)
...
if __name__ == '__main__':
    ...
    #             #  训练
    #             tfrelu = Ten_train(train_x_data, train_y_data, predict_x_data,'O3-8H', hiddennodes=nodeNum,batch_size=batchSize)
    #             minumber = data[4][1]
    #             maxumber = data[4][0]
    #
    #             #训练和测试的真实输出
    ...
    #             key = str(ly)+'and'+str(nodeNum)+'and'+str(batchSize)
    #             aucdict.setdefault(key, auc)
    #             print(key+","+str(auc))
    #             costfig(tfrelu[2])
    # batchSizefigure(aucdict)
    tfrelu = Ten_train(train_x_data, train_y_data, predict_x_data, 'O3-8H')
    minumber = data[4][1]
    maxumber = data[4][0]
