...
class BPNN():
    ...
    def __init__(self, train_in, train_out,nodeNums,learn_rate=0.03, son_samples=10, iter_times=500,middle_name='Sigmoid', last_name='Sigmoid', cost_func='Lsm', norr=0.00002, break_error=0.0005):
    ...
    def train_adam(self,mom=0.9, prop=0.9):
        ...
        while iter < self.iter_times:
            ...
            # 打印误差值
            errortrain = eval(self.cost_func)(trout, z[len(self.all_layer) - 1])
            print('NO2: '+u'第%s代总体误差：%.9f' % (iter, errortrain))
...
if __name__ == "__main__":
    errordic = {}
    data=R_data.get('NO2')
    # data[0] = np.array([[row[i] for i in range(0, 12) if i != 2 and i != 3] for row in data[0]])
    # data[2] = np.array([[row[i] for i in range(0, 12) if i != 2 and i != 3] for row in data[2]])
