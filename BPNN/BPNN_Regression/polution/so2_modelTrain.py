...
class BPNN():
    ...
    def __init__(self, train_in, train_out,nodeNums,learn_rate=0.03, son_samples=50, iter_times=500,middle_name='Sigmoid', last_name='Sigmoid', cost_func='Lsm', norr=0.00002, break_error=0.0004):
    ...
    def train_adam(self,mom=0.9, prop=0.9):
        ...
        while iter < self.iter_times:
            ...
            # 打印误差值
            errortrain = eval(self.cost_func)(trout, z[len(self.all_layer) - 1])
            print('SO2: '+u'第%s代总体误差：%.9f' % (iter, errortrain))
...
if __name__ == "__main__":
    ...
    for nodeNums in range(5,20):
        ...
        predict_in = predict_y_data_tran
        predict_out = bpnn.predict(predict_x_data)*(maxumber - minumber) + minumber#测试集预测结果
        #random_predict_data = select(predict_in, predict_out, 200) #从测试集拿出200条
        ...
        for k in range(len(error)):
            if error[k] <= 0.3:
                count = count+1
        auc = count/len(predict_in)
        errordic.setdefault(nodeNums,auc)
        if auc>=0.75:
            break
        # 数据多影响展示，随机挑选100条数据
