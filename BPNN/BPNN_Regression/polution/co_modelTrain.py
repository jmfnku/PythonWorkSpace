...
class BPNN():
    ...
    def train_adam(self,mom=0.9, prop=0.9):
        ...
        while iter < self.iter_times:
            ...
            # 打印误差值
            errortrain = eval(self.cost_func)(trout, z[len(self.all_layer) - 1])
            print('CO: '+u'第%s代总体误差：%.9f' % (iter, errortrain))
...
if __name__ == "__main__":
    errordic = {}
    data=R_data.get('CO')
    data[0] = np.array([[row[i] for i in range(0, 12) if i != 1 and i != 10] for row in data[0]])
    data[2] = np.array([[row[i] for i in range(0, 12) if i != 1 and i != 10] for row in data[2]])
