...
class BPNN():
    ...
    def __init__(self, train_in, train_out,nodeNums=8,learn_rate=0.001, son_samples=50, iter_times=2000,
                 middle_name='Sigmoid', last_name='Sigmoid', cost_func='Lsm', norr=0.0001, break_error=0.002):
    ...
    def train_adam(self,mom=0.8, prop=0.8):
        # 采用随机小批量梯度下降
        # 首先结合输入和输出数据
        ...
        while iter < self.iter_times:
            ...
            print('O3-8H: '+u'第%s代总体误差：%.9f' % (iter, errortrain))
...
if __name__ == "__main__":
    ...
    data[0] = np.array([[row[i] for i in range(0, 12) if i != 7] for row in data[0]])
    data[2] = np.array([[row[i] for i in range(0, 12) if i != 7] for row in data[2]])
